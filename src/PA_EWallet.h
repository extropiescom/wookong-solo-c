...
#define PAEW_RET_DEV_OP_CANCEL				0x80000008 ///<current operation on device is canceled
...
#define PAEW_RET_DEV_FUNC_INVALID			0x8000003D ///<user-defined device function invalid
///@}
...
#define PAEW_DEV_TYPE_USERDEF	0x02 ///<user-defined device
/// @}
...
#define PAEW_DERIVE_PATH_PREFIX_COUNT    4 ///<when generate seed in remote way, users need to pass derive path with length of #PAEW_DERIVE_PATH_PREFIX_COUNT (including first 0 indicating m) to function(make devices having ability of deriving coin addresses without storing private key, see also: https://github.com/bitcoin/bips/blob/master/bip-0032.mediawiki: Knowing an extended private key allows reconstruction of all descendant private keys and public keys, and knowing an extended public keys allows reconstruction of all descendant non-hardened public keys)

#define PAEW_SIGN_INFO_MAX_LEN		1024 ///<max length of sign info, for PAEW_GetMultiSignInfo_Remote() and PAEW_GetSingleSignInfo_Remote()
/// @}
...
typedef struct _PAEW_FWVersion
{
	...
	unsigned char	pbBLEVersion[PAEW_FW_VERSION_BLEVERSION_LEN]; ///<BLE version
...
/**
* \defgroup group_user_def_dev User-defined device
*/
/// @{
typedef void * USERDEF_DEV_HANDLE; ///<used for device implement
typedef int(*FuncType_UserDef_EnumDevice)(void* const pDevSpecContext, char * const szDevNameList, size_t * const pnBufferLen, size_t *pnDevCount); ///<enumerate device, szDevNameList should be a input buffer, which size is *pnBufferLen. If success, szDevNameList should be as this format: devName1 + \0 + devName2 + \0 + ... + devNameN + \0 + \0, *pnDevCount should return count of valid device
typedef int(*FuncType_UserDef_Connect)(const char * const szDevName, void* const pDevSpecContext, USERDEF_DEV_HANDLE * const phDevHandle); ///<connect device by device name, return a device handle
typedef int(*FuncType_UserDef_Disconnect)(const USERDEF_DEV_HANDLE hDevHandle, void* const pDevSpecContext); ///<disconnect device by a device handle
typedef int(*FuncType_UserDef_SendCommand)(const USERDEF_DEV_HANDLE hDevHandle, void* const pDevSpecContext, const unsigned char * const pbSendData, const size_t nSendDataLen, unsigned char * pbReceiveData, size_t * const pnReceiveDataLen); ///<communicate device using device handle

typedef struct _USERDEF_DEV_FUNCTION_LIST
{
	FuncType_UserDef_EnumDevice		pFuncEnumDeviceEx;
	FuncType_UserDef_Connect		pFuncConnectEx;
	FuncType_UserDef_Disconnect		pFuncDisconnectEx;
	FuncType_UserDef_SendCommand	pFuncSendCommandEx;
} USERDEF_DEV_FUNCTION_LIST;
/// @}
...
///@{
/**
* \brief		Import user-defined device definition, only support import ONCE, if import multiple times, the former would be replaced
* \param[in]	pDevFuncList	user should define a structure of USERDEF_DEV_FUNCTION_LIST, which stored function pointer to manipulate device
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_ImportUserDefDevice(const USERDEF_DEV_FUNCTION_LIST * const pDevFuncList);

/**
* \brief		Clear user-defined device definition, MUST be called ONCE before application exit if PAEW_ImportUserDefDevice is invoked
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_RemoveUserDefDevice();
...
* This function may be blocked and wait user to press button, and user must call PAEW_FreeContext() and then PAEW_InitContext() (or other init functions) to reconnect device before doing other operations
* \param[in]		pPAEWContext	library context, shouldn't be NULL
* \param[in]		nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \return			#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
...
/**
* \brief			Update user COS, and user must call PAEW_FreeContext() and then PAEW_InitContext() (or other init functions) to reconnect device before doing other operations
* \param[in]		pPAEWContext	library context, shouldn't be NULL
* \param[in]		nDevIndex		index of device, valid range of value is [0, nDevCount-1]
...
/**
* \brief		Update user COS using progress callback and context, support resume from breakpoint of last unfinished procedure, and user must call PAEW_FreeContext() and then PAEW_InitContext() (or other init functions) to reconnect device before doing other operations
* \param[in]	pPAEWContext		library context, shouldn't be NULL
* \param[in]	nDevIndex			index of device, valid range of value is [0, nDevCount-1]
...
/**
* \brief		Update user COS and BLE COS using progress callback and context, support resume from breakpoint of last unfinished procedure, and user must call PAEW_FreeContext() and then PAEW_InitContext() (or other init functions) to reconnect device before doing other operations
* \param[in]	pPAEWContext		library context, shouldn't be NULL
* \param[in]	nDevIndex			index of device, valid range of value is [0, nDevCount-1]
* \param[in]	bRestart			0 means resume breakpoint of last procedure, 1 means restart COS download
* \param[in]	pbUserCOSData		user COS data
...
/**
* \defgroup group_remote_functions Remote functions
*/
///@{

/**
* \brief			Generate seed to initialize device, remote version
* \param[in]		pPAEWContext	library context, shouldn't be NULL
* \param[in]		nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]		nCoinTypeCount	coin type count
* \param[in]		pnCoinTypes		coin type array, element amount is \p nCoinTypeCount
* \param[in]		ppuiDerivePath	derive paths corresponding to coin types of pnCoinTypes, element amount is \p nCoinTypeCount
* \param[in]		pnDerivePathLen	length of each derive path, element amount is \p nCoinTypeCount
* \param[in]		nN				system parameter, count of parts which seed be separated into, 4<=N<=7, and must be equal to device count currently connected to host device
* \param[in]		nT				system parameter, count of devices needed when restore seed, 2<=M<=N
* \return			#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GenerateSeed_Remote(void * const pPAEWContext, const size_t nDevIndex, const size_t nCoinTypeCount, const unsigned char * const pnCoinTypes, const uint32_t ** const ppuiDerivePath, const size_t * const pnDerivePathLen, const uint8_t nN, const uint8_t nT);

/**
* \brief			Get trade address, remote version, _MUST_ be called before PAEW_GetSignInfo_Remote()
* \param[in]		pPAEWContext		library context, shouldn't be NULL
* \param[in]		nDevIndex			index of device, valid range of value is [0, nDevCount-1]
* \param[in]		nCoinType			coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]		nShowOnScreen		whether coin address showed on device screen, 0 means not shown, 1 means show address on screen
...
* \return			#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GetTradeAddress_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const unsigned char nShowOnScreen, const uint32_t * const puiDerivePath, const size_t nDerivePathLen, unsigned char * const pbTradeAddress, size_t * const pnTradeAddressLen);

/**
* \brief		Set transaction to device, remote version
* \param[in]	pPAEWContext	library context, shouldn't be NULL
* \param[in]	nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]	nCoinType		coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]	nUTXOCount		for BTC/LTC/USDT/NEO..., count of UTXOs, must equal to input number of transaction to be singed
* \param[in]	ppbUTXO			for BTC/LTC/USDT/NEO..., each of which contains one UTXO data
* \param[in]	pnUTXOLen		for BTC/LTC/USDT/NEO..., each of which contains length of UTXO data corresponding to the same position of ppbUTXO
* \param[in]	pbCurrentTX		transaction data to be signed
* \param[in]	nCurrentTXLen	length of transaction data to be signed
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_SetTX_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const size_t nUTXOCount, const unsigned char * const * const ppbUTXO, const size_t * const pnUTXOLen, const unsigned char * const pbCurrentTX, const size_t nCurrentTXLen);

/**
* \brief		Get multiple sign infos from device (for BTC/LTC/USDT/NEO... which has multiple inputs), which is needed by PAEW_GetMultiSignResult_Remote() when signing transaction, remote version, _MUST_ be called after PAEW_GetTradeAddress_Remote()
* \param[in]	pPAEWContext	library context, shouldn't be NULL
* \param[in]	nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]	nCoinType		coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]	puiDerivePath	derive path, shouldn't be NULL, and first element must be 0, indicating m (from seed to master private key), shouldn't be NULL
* \param[in]	nDerivePathLen	count of derive path elements, not less than 1
* \param[in]	nUTXOCount		count of UTXOs, must equal to input number of transaction to be singed
* \param[out]	ppbSignInfo		array of buffers containing output sign infos, each length should be #PAEW_SIGN_INFO_MAX_LEN when input
* \param[out]	pnSignInfoLen	each of which contains length of buffer corresponding to the same position of \p ppbSignInfo when input, and contains length of result when output
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GetMultiSignInfo_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const uint32_t * const puiDerivePath, const size_t nDerivePathLen, const size_t nUTXOCount, unsigned char * const * const ppbSignInfo, size_t * const pnSignInfoLen);

/**
* \brief		Get multiple signatures from device (for BTC/LTC/USDT/NEO...), remote version
* \param[in]	pPAEWContext	library context, shouldn't be NULL
* \param[in]	nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]	nCoinType		coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]	nSingInfoCount	count of sign infos, each device produces one sign infos, this parameter means the amount of devices which has confirmed transaction using PAEW_GetMultiSignInfo_Remote()
* \param[in]	nUTXOCount		count of UTXOs, must equal to input number of transaction to be singed, and means the amount of elements of pppbSignInfo[i]
* \param[in]	pppbSignInfo	sign infos from PAEW_GetMultiSignInfo_Remote(), should be constructed in form of pppbSignInfo[nSingInfoCount][nUTXOCount]
* \param[in]	ppnSignInfoLen	contains lengths of each element of sign infos (pppbSignInfo[i][j])
* \param[out]	ppbTXSig		array of buffers containing output signatures, each length should be #PAEW_EOS_SIG_MAX_LEN when input
* \param[out]	pnTXSigLen		each of which contains length of buffer corresponding to the same position of \p ppbTXSig when input, and contains length of result when output
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GetMultiSignResult_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const size_t nSingInfoCount, const size_t nUTXOCount, const unsigned char *** const pppbSignInfo, const size_t ** const ppnSignInfoLen, unsigned char * const * const ppbTXSig, size_t * const pnTXSigLen);

/**
* \brief		Get single sign info from device, which is needed by PAEW_GetSingleSignResult_Remote() when signing transaction, remote version, _MUST_ be called after PAEW_GetTradeAddress_Remote()
* \param[in]	pPAEWContext	library context, shouldn't be NULL
* \param[in]	nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]	nCoinType		coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]	puiDerivePath	derive path, shouldn't be NULL, and first element must be 0, indicating m (from seed to master private key), shouldn't be NULL
* \param[in]	nDerivePathLen	count of derive path elements, not less than 1
* \param[out]	pbSignInfo		buffer containing output sign info, which length should be #PAEW_SIGN_INFO_MAX_LEN when input
* \param[out]	pnSignInfoLen	contains length of \p pbSignInfo when input, and contains length of result when output
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GetSingleSignInfo_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const uint32_t * const puiDerivePath, const size_t nDerivePathLen, unsigned char * const pbSignInfo, size_t * const pnSignInfoLen);

/**
* \brief		Get single signature from device, remote version
* \param[in]	pPAEWContext	library context, shouldn't be NULL
* \param[in]	nDevIndex		index of device, valid range of value is [0, nDevCount-1]
* \param[in]	nCoinType		coin type, valid values are PAEW_COIN_TYPE_XXX (\ref group_coin_type)
* \param[in]	nSingInfoCount	count of sign infos, each device produces one sign info, this parameter means the amount of devices which has confirmed transaction using PAEW_GetSingleSignInfo_Remote()
* \param[in]	ppbSignInfo		sign infos from PAEW_GetSingleSignInfo_Remote(), should be constructed in form of ppbSignInfo[nSingInfoCount]
* \param[in]	pnSignInfoLen	contains lengths of each element of sign infos (ppbSignInfo[i])
* \param[out]	pbTXSig			buffer containing output signature, length should be #PAEW_EOS_SIG_MAX_LEN when input
* \param[out]	pnTXSigLen		contains length of \p pbTXSig when input, and contains length of result when output
* \return		#PAEW_RET_SUCCESS means success, and other value means failure (\ref group_retvalue)
*/
int EWALLET_API PAEW_GetSingleSignResult_Remote(void * const pPAEWContext, const size_t nDevIndex, const unsigned char nCoinType, const size_t nSingInfoCount, const unsigned char ** const ppbSignInfo, const size_t * const pnSignInfoLen, unsigned char * const pbTXSig, size_t * const pnTXSigLen);
